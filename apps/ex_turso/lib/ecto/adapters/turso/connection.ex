if Code.ensure_loaded?(Ecto.Adapters.SQL.Connection) do
  defmodule Ecto.Adapters.Turso.Connection do
    @moduledoc false
    ...
    import Ecto.Adapters.Turso.DataType
    ...
    defp normalize_database(opts) do
      case Keyword.get(opts, :database) do
        :memory -> Keyword.put(opts, :database, ":memory:")
        _ -> opts
      end
    end
    ...
    def start_link(opts) do
      ...
      DBConnection.start_link(Turso.Connection, opts)
    end
    ...
    def child_spec(options) do
      ...
      DBConnection.child_spec(Turso.Connection, options)
    end

    @impl true
    def prepare_execute(conn, name, sql, params, options) do
      query = build_query(sql, name)

      case DBConnection.execute(conn, query, params, options) do
        {:ok, returned_query, result} -> {:ok, returned_query, to_ecto_result(result)}
        {:error, %Turso.Error{}} = error -> error
        {:error, err} -> raise err
      end
    end

    @impl true
    def execute(conn, %Turso.Query{} = query, params, options) do
      case DBConnection.execute(conn, query, params, options) do
        {:ok, returned_query, result} -> {:ok, returned_query, to_ecto_result(result)}
    ...
    def execute(conn, sql, params, options) when is_binary(sql) or is_list(sql) do
      case query(conn, sql, params, options) do
        {:ok, result} -> {:ok, result}
        other -> other
      end
    end
    ...
    def query(conn, sql, params, options) do
      ...
      case DBConnection.execute(conn, query, params, options) do
        {:ok, _, result} -> {:ok, to_ecto_result(result)}
        other -> other
      end
    end
    ...
    def query_many(_conn, _sql, _params, _opts) do
      raise RuntimeError, "query_many is not supported in the Turso adapter"
    end

    @impl true
    def stream(_conn, _sql, _params, _options) do
      raise RuntimeError, "streams are not supported in the Turso adapter"
    end

    defp build_query(sql, name \\ "") do
      statement = IO.iodata_to_binary(sql)
      %Turso.Query{name: name, statement: statement, command: command_for(statement)}
    end

    defp command_for(statement) do
      if returns_rows?(statement), do: :query_rows, else: :execute
    end
    ...
    defp returns_rows?(statement) do
      ...
      String.starts_with?(upcased, "SELECT") or
        String.starts_with?(upcased, "WITH") or
        String.starts_with?(upcased, "PRAGMA") or
        String.starts_with?(upcased, "EXPLAIN") or
        String.match?(upcased, ~r/\bRETURNING\b/)
    end

    defp to_ecto_result(%Turso.Result{columns: columns, rows: rows, num_rows: num_rows}) do
      %{columns: columns, rows: rows_to_lists(rows, columns), num_rows: num_rows}
    end

    defp rows_to_lists(nil, _columns), do: nil
    defp rows_to_lists(rows, nil), do: rows

    defp rows_to_lists(rows, columns) do
      Enum.map(rows, fn
        row when is_map(row) -> Enum.map(columns, &Map.get(row, &1))
        row when is_list(row) -> row
      end)
    end

    # we want to return the name of the underlying index that caused
    # the constraint error, but in Turso as far as I can tell there
    ...
    @impl true
    def to_constraints(
          %Turso.Error{message: "UNIQUE constraint failed: index " <> constraint},
          _opts
        ) do
    ...
    def to_constraints(
          %Turso.Error{message: "UNIQUE constraint failed: " <> constraint},
          _opts
        ) do
      [unique: constraint_name_hack(constraint)]
    end

    def to_constraints(%Turso.Error{message: "FOREIGN KEY constraint failed"}, _opts) do
      # unfortunately we have no other data from Turso
      [foreign_key: nil]
    end

    def to_constraints(
          %Turso.Error{message: "CHECK constraint failed: " <> name},
          _opts
        ) do
      [check: name]
    end
    ...
    def all(%Ecto.Query{lock: lock}) when lock != nil do
      raise ArgumentError, "locks are not supported by Turso"
    end
    ...
    def delete_all(%Ecto.Query{joins: [_ | _]}) do
      ...
      raise ArgumentError, "JOINS are not supported on DELETE statements by Turso"
    end
    ...
    def execute_ddl({_command, %Table{options: options}, _}) when is_list(options) do
      raise ArgumentError, "Turso adapter does not support keyword lists in :options"
    end
    ...
    def execute_ddl({_, %Index{concurrently: true}}) do
      raise ArgumentError, "`concurrently` is not supported with Turso"
    end

    def execute_ddl({_, %Index{only: true}}) do
      raise ArgumentError, "`only` is not supported with Turso"
    end

    def execute_ddl({_, %Index{include: [_ | _]}}) do
      raise ArgumentError, "`include` is not supported with Turso"
    end

    def execute_ddl({_, %Index{using: x}}) when not is_nil(x) do
      raise ArgumentError, "`using` is not supported with Turso"
    end

    def execute_ddl({_, %Index{nulls_distinct: x}}) when not is_nil(x) do
      raise ArgumentError, "`nulls_distinct` is not supported with Turso"
    end
    ...
    def execute_ddl(keyword) when is_list(keyword) do
      raise ArgumentError, "Turso adapter does not support keyword lists in execute"
    end

    def execute_ddl({:create, %Constraint{}}) do
      raise ArgumentError, "Turso does not support ALTER TABLE ADD CONSTRAINT."
    end

    def execute_ddl({:drop, %Constraint{}, _mode}) do
      raise ArgumentError, "Turso does not support ALTER TABLE DROP CONSTRAINT."
    end
    ...
    defp on_conflict({:replace_all, _, {:constraint, _}}, _header) do
      raise ArgumentError, "Upsert in Turso does not support ON CONSTRAINT"
    end

    defp on_conflict({:replace_all, _, []}, _header) do
      raise ArgumentError, "Upsert in Turso requires :conflict_target"
    end
    ...
    def insert_each(values, counter) do
      intersperse_reduce(values, ?,, counter, fn
        nil, _counter ->
          raise ArgumentError,
                "Cell-wise default values are not supported on INSERT statements by Turso"
    ...
    defp distinct(%ByExpr{expr: exprs}, _sources, query) when is_list(exprs) do
      raise Ecto.QueryError,
        query: query,
        message: "DISTINCT with multiple columns is not supported by Turso"
    end
          ...
          case elem(sources, idx) do
            {source, _, nil} ->
              raise Ecto.QueryError,
                query: query,
                message: """
                Turso does not support selecting all fields from #{source} \
    ...
    defp update_op(:push, _quoted_key, _value, _sources, query) do
      raise Ecto.QueryError,
        query: query,
        message: "Arrays are not supported for Turso"
    end
    ...
    defp update_op(command, _quoted_key, _value, _sources, query) do
      raise Ecto.QueryError,
        query: query,
        message: "Unknown update operation #{inspect(command)} for Turso"
    end
    ...
    defp assert_valid_join(%JoinExpr{hints: hints}, query) when hints != [] do
      raise Ecto.QueryError,
        query: query,
        message: "join hints are not supported by Turso"
    end
    ...
    defp join_qual(mode, query) do
      raise Ecto.QueryError,
        query: query,
        message: "join `#{inspect(mode)}` not supported by Turso"
    end
    ...
    defp order_by_expr({dir, expression}, sources, query) do
      ...
      case dir do
        ...
        _ ->
          raise Ecto.QueryError,
            query: query,
            message: "#{dir} is not supported in ORDER BY in Turso"
      end
    end
    ...
    defp combination({:except_all, query}, _) do
      raise Ecto.QueryError,
        query: query,
        message: "Turso does not support EXCEPT ALL"
    end

    defp combination({:intersect_all, query}, _) do
      raise Ecto.QueryError,
        query: query,
        message: "Turso does not INTERSECT ALL"
    end

    def lock(query, _sources) do
      raise Ecto.QueryError,
        query: query,
        message: "Turso does not support locks"
    end
    ...
    # workaround for the fact that Turso as of 3.35.4 does not support specifying table
    ...
    defp expr({:fragment, _, [kw]}, _sources, query)
      ...
      raise Ecto.QueryError,
        query: query,
        message: "Turso adapter does not support keyword or interpolated fragments"
    end
    ...
    defp expr({:datetime_add, _, [datetime, count, interval]}, sources, query) do
      format =
        case Application.get_env(:ex_turso, :datetime_type) do
          :text_datetime ->
            "%Y-%m-%d %H:%M:%f000Z"
    ...
    defp expr({:ilike, _, [_, _]}, _sources, query) do
      raise Ecto.QueryError,
        query: query,
        message: "ilike is not supported by Turso"
    end
    ...
    defp expr({:count, _, [{:&, _, [_]}]}, _sources, query) do
      raise Ecto.QueryError,
        query: query,
        message: "The argument to `count/1` must be a column in Turso"
    end
    ...
    defp expr(list, _sources, query) when is_list(list) do
      raise Ecto.QueryError,
        query: query,
        message: "Array literals are not supported by Turso"
    end
    ...
    def interval(_, "microsecond", _sources) do
      raise ArgumentError,
            "Turso does not support microsecond precision in datetime intervals"
    end
    ...
    # If we are adding a DATETIME column with the NOT NULL constraint, Turso
    ...
    defp default_expr({:ok, value}) when is_map(value) or is_list(value) do
      library = Application.get_env(:ex_turso, :json_library, Jason)
    ...
    defp quote_table(prefix, _name) when is_atom(prefix) or is_binary(prefix) do
      raise ArgumentError, "Turso does not support table prefixes"
    end
