defmodule Concord.Performance.KVOperationsBenchmark do
  ...
  def run_basic_kv_benchmarks do
    ...
    benchee_run(
      %{
        "put_small_value" => fn ->
          Concord.put(
            "bench:small:#{:erlang.unique_integer()}",
            "small_value_#{System.unique_integer()}"
          )
        end,
        "get_small_value" => fn ->
  ...
  def run_memory_usage_benchmarks do
    ...
    for size <- sizes do
      ...
      values =
        for i <- 1..size do
          # ~100 bytes per value
          value = "data_" <> String.duplicate("x", 100)
      ...
      IO.puts(
        "#{size} items: #{format_memory(memory_after)} (~#{Float.round(memory_per_item, 2)} bytes/item)"
      )
    end
  ...
  defp analyze_ets_performance do
    ...
    for {type, subtype} <- table_types do
      ...
      IO.puts(
        "  Insert 1000 items: #{insert_time}μs (#{Float.round(1000_000 / insert_time, 2)} ops/sec)"
      )

      IO.puts(
        "  Lookup 1000 items: #{lookup_time}μs (#{Float.round(1000_000 / lookup_time, 2)} ops/sec)"
      )
